# ASDM Toolset - Task Planner & Executor

toolset-id: task-planner-executor
toolset-name: Task Planner & Executor
version: 0.0.2
...
Task Planner & Executor (toolset-id: task-planner-executor) is a ASDM toolset for planning and executing tasks for a workspace. It takes a feature description as input and generates a task plan for the feature, then execute the tasks for the feature. During the execution phase, it will update the task plan with the current status of each task.
...
```shell
Follow instructions in .asdm/toolsets/task-planner/INSTALL.md
```
## Features

Main features of Task Planner & Executor:
...
- Create `.asdm/workspace/features` directory for Task Planner's workspace
...
- Create shortcuts commands for `Task Planner` in provider's entry point, e.g. `.claude/commands`, `.github/prompts`, `.codebuddy/commands` etc.

## Toolset Workflow

Once `Task Planner` is installed, user can use the following commands to plan tasks for the current workspace:

- `/asdm-task-planning`: plan tasks for the current workspace
- `/asdm-task-breakdown`: breakdown tasks for the current workspace
- `/asdm-task-execution`: execute tasks for the current workspace

## Toolset Structure

The Task Planner toolset has the following structure:

```
.asdm/
└── toolsets/
    └── task-planner-executor/              ## Task Planner & Executor toolset
        ├── INSTALL.md                      ## Installation instructions for the toolset
        ├── README.md                       ## Current document
        └── actions/                        ## Instructions for Task Planner & Executor
            ├── asdm-task-planning          ## Instruction for planning tasks for a workspace
            ├── asdm-task-breakdown          ## Instruction for breaking down tasks for a workspace
            └── asdm-task-execution         ## Instruction for executing tasks for a workspace
        └── spec/                           ## Spec documents for Task Planner & Executor
...
## Toolset Workspace
The Task Planner toolset has the following workspace structure:
